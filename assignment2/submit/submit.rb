...
if dirs[0] != "assignment2" && dirs[1] != "submit"
  $stderr.puts "You have not changed to the 'submit' directory of the 'assignment2' directory"
  exit 1
end
...
file = id + ".tar.gz"
begin
  FileUtils.remove_dir "assignment2", true
  FileUtils.rm file
rescue => e
end

FileUtils.mkdir "assignment2"
FileUtils.cd('..') do
  begin
    (["Makefile"] + Dir.glob("*.[ch]")).each do |file|
      FileUtils.cp file, "submit/assignment2"
    end
  rescue => e
