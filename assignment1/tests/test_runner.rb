...
@tests = [
  {tc: 'Run a command with no arguments', depends: :EXTERNAL, cmd: %x{which uname}, expected: %x{uname}, explanation: "Unexpected output from running the a command"},
  {tc: 'Check if fork() is implemented', depends: :EXTERNAL, cmd: %x{which ps}, expected: 'lsh', explanation: "Command not executed in fork()"},
  {tc: 'Run a command with a single argument', depends: :EXTERNAL, cmd: %x{which uname} + ' -n', expected: %x{uname -n}, explanation: "No support for passing command line arguments (1 arg)"},
  {tc: 'Run a command with three arguments', depends: :EXTERNAL, cmd: %x{which expr} + ' 1 + 1', expected: '^2', explanation: "No support for passing command line arguments (3 args)"},
  {tc: 'Run a commmand with multiple arguments', depends: :EXTERNAL, cmd: %x{which expr} + ' 5 * 6 + 7 / 2 - 8 * 3', expected: '^9', explanation: "No support for passing command line arguments (11 args)"},
  {tc: 'Check if PATH lookup is implemented for a command that exists', depends: :EXTERNAL, cmd: %x{which uname}, expected: 'Linux', explanation: "Command in PATH but not executed "},
  ...
  {tc: 'Check if relative path commands will run', depends: :EXTERNAL, cmd: relative_path_to(%x{which uname}), expected: 'Linux', explanation: "Relative path command not executed "},
